...
- **Reusable Catalogue** lives at `app/_components/catalogue/`: `Catalogue` (interactive — search, sidebar with vertical category radiogroup + sort, mobile collapses to two `Select`s, results count, URL state, empty state, image cards), `CatalogueStatic` (server-rendered fallback used inside `<Suspense>` so the static prerender still contains crawlable item links), `CatalogueItem` (detail-page wrapper: side-by-side preview + heading/controls/CTA in the right column, with tabs below and a programmatic `navigateToTab` handle for CTAs that jump tabs). The same pair powers a future parts catalog.
- **Layout matches the legacy** rather than the chip-row + 6xl-container shape it had on first pass: `maxW="8xl"` outer container; on `lg+` a 14rem left sidebar (Categories vertical list, Sort `Select` below) sits beside the search bar + 3-column grid; cards are image-forward (`borderRadius=xl`, `boxShadow=md`, `objectFit=cover`, no description, no surrounding border) with the name centered beneath. Detail page drops the page-level `Title` and renders the heading inline above the controls in the right column on desktop (above the preview on mobile), so the controls + "View cutting plan" CTA line up with the 4/3 preview.
- Design images come from `gridkit-products/products/<id>/<id>.png`. `app/_lib/design-images.ts` maps id → static next/image import; the cards render via `<Image unoptimized />` so the global Cloudinary loader is bypassed for these local assets.
- Search/filter/sort state syncs through `?q=`, `?f=`, `?s=` URL params via `useRouter`/`useSearchParams` (no `use-query-params` dependency). The whole `Catalogue` is wrapped in `<Suspense>` on the page since `useSearchParams()` opts a static page into dynamic rendering otherwise.
