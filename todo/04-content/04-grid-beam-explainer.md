...
## Steps
- [ ] Start from `node-modules/apps/gridkit/pages/about.tsx` — review structure (sections, diagrams).
- [ ] Outline the new page:
...
- [ ] Draft each section's prose. Aim for ~150 words per section.
- [ ] Identify or commission diagrams (the legacy site has some good ones — re-use where possible).
- [ ] Cross-check facts: hole spacing? Beam cross-section? Common bolt sizes? Don't write authoritatively about specifics without verifying.
- [ ] Sensitivity check: is there anything that implies "you must buy this from a specific company"? Rewrite.
- [ ] Pass to Stream 01 task 04 for JSX implementation.
