// Ported from https://github.com/villagekit/node-modules/blob/fce357d/apps/gridkit/public/icon.svg
// Originally Grid Kit's brand mark; carried forward to Grid Beam by the same founder.

interface CubeLogoProps {
  size: number | string
  /** Set to a string for an accessible name; omit (or pass `null`) for decorative use. */
  ariaLabel?: string | null
}

export function CubeLogo(props: CubeLogoProps) {
  const { size, ariaLabel = 'Grid Beam cube logo' } = props
  const a11yProps =
    ariaLabel == null || ariaLabel === ''
      ? { 'aria-hidden': true as const }
      : { role: 'img' as const, 'aria-label': ariaLabel }
  return (
    // biome-ignore lint/a11y/noSvgWithoutTitle: decorative when ariaLabel is null (link's text label provides the accessible name); a `<title>` element would be redundant for AT users.
    <svg
