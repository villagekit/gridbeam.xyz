...
- **The route owns its page and the shell owns the rest.** A route lives under `app/<path>/` and consumes the shell (`app/layout.tsx`, `app/_components/Site*`, `app/_lib/nav.ts`) and the shared components under `app/_components/`; a difference a shell change closes is filed on `shell`, never per route. A helper the current code invented is a difference (the `parity` skill's code family); the legacy author's pattern is the standard a new edge is measured against.
...
- Open: the worker-model gradation for `/orchestrate` (whether a mechanical slice runs on Sonnet and an in-place fix on Opus) is a design conversation the operator has opened and not decided; until a decision supersedes `077cef20`, every worker is Fable.
...
- `kipu new plan --title "..." --parent <id>` for work you discover; `kipu note <id> -` for a dated remark; `kipu finish <id> --outcome -` when done. Every mutation stages; the agent commits, as Plans vs. reality says (`077cef20`).
