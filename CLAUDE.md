...
|------|--------------------------|---------|
| `villagekit/ui` (`@villagekit/ui`, `../ui`) | **npm dependency** in `package.json`; the published package ships compiled ESM and resolves `.` to `dist/index.js`, so `next.config.ts` lists no `transpilePackages` (only the sibling's top-level `exports` point at `src/`). To bump: `pnpm update @villagekit/ui --latest`, then the gate, commit `package.json` + `pnpm-lock.yaml`. The sibling is for fixes that land upstream; a fix is seen here before its publish through an uncommitted `pnpm.overrides` of `@villagekit/ui` to `file:../ui` (`link:../ui` does not resolve under `next dev --turbopack`; `file:` copies the sibling at install time, so each sibling edit needs a `pnpm install` before `pnpm dev` sees it; the copy resolves to `src/*.ts`, which needs an uncommitted `transpilePackages: ['@villagekit/ui']` in `next.config.ts` beside the override, without which `next build`'s worker dies with SIGILL and `next dev` dies compiling the first route), reverted by path before the commit. | Public: `villagekit/ui` |
| `villagekit/gridkit` (`@villagekit/parameters`, `part`, `part-fastener`, `part-gridbeam`, `part-gridpanel`, `plugin-smart-fasteners`, `product`, `product-kit`, `sandbox`; `../gridkit`) | **npm dependencies** in `package.json`. Same `dist/` shape and bump flow as `@villagekit/ui`, so no transpile. The sibling is for fixes that land upstream. | Public: `villagekit/gridkit` |
...
- `app/`: the Next.js app router, one directory per route (`about`, `contact`, `designs`, `faq`, `legal`, `stories`, `subscribe`, `suppliers`, `tools`, `tools-and-resources`), the six story pages as `app/stories/<slug>/page.mdx` under `app/_components/layouts/StoriesLayout.tsx`, the shell (`layout.tsx`, `theme.ts`, `not-found.tsx`, `icon.svg`, `robots.ts`, `sitemap.ts`), the shared components under `app/_components/` and the data and helpers under `app/_lib/` (the designs catalog, the generated designs data, the stories catalog, the nav, the site-wide Open Graph fields, the Cloudinary loader, the URL state).
