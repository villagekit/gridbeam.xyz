...
The route `/tools/cutting-planner` is at parity with the legacy site: every difference on it is `fixed` or `sanctioned`, and the operator has reviewed it on `pnpm dev` against the legacy site. Decision `ee86d68a`.
...
`kipu list --collection difference --filter route=/tools/cutting-planner --json` shows no `open` or `regression` item, and the operator finishes this record after reviewing the route.
