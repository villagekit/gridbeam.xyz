...
- 2026-09-26: Plan a849fca1426c dropped the hand transpilePackages list: no published @villagekit/* package at the pinned versions needs it (each resolves `.` to `dist/index.js`; `next build` and `pnpm dev` pass with the list gone, entry by entry and all at once), so the Current field's hand-maintained list is stale. Legacy's `monorepo` discovery still has no `packages/` to read here, and legacy's own hand entries (`@villagekit/part-gridpanel` for a deadbeef import, `@swc/wasm`) name reasons the published packages no longer carry.
